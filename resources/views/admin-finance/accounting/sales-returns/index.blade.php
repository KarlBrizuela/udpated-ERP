<x-app-layout :title="'Sales Returns'" :sidebar="$sidebar ?? 'admin-finance'" :role="$role ?? 'Finance Manager'">
    @push('styles')
    <style>
            ...
            <div class="col-12">
                ...
                <div class="card border-0 shadow-sm" style="border-radius: 8px;">
                    <div class="card-header bg-white border-bottom py-3 d-flex flex-wrap justify-content-between align-items-center gap-3">
                        <div>
                            <h4 class="fs-18 mb-0 fw-bold text-dark" style="letter-spacing: -0.5px;">Sales Returns Log</h4>
                            <p class="text-muted small mb-0">Record and track returned sales invoices and inventory restocks.</p>
                        </div>
                        <div class="d-flex align-items-center gap-3">
                            <form action="{{ route('admin-finance.accounting.sales-returns.index') }}" method="GET" class="search-container">
                                <div class="input-group input-group-sm" style="width: 320px;">
                                    ...
                                    <input type="text" name="search" class="form-control border-start-0 custom-search-input" placeholder="Search Return or Order..." value="{{ request('search') }}" style="border-top-right-radius: 0 !important; border-bottom-right-radius: 0 !important;">
                                    @if(request('search'))
                                        <a href="{{ route('admin-finance.accounting.sales-returns.index') }}" class="btn btn-sm text-white d-inline-flex align-items-center justify-content-center" style="background-color: #64748b; height: 38px; border-top-right-radius: 4px; border-bottom-right-radius: 4px; padding: 0 16px; font-weight: 600; border: 1px solid #64748b;">
                                            Clear
                                        </a>
                                    @else
                            ...
                            <a href="{{ route('admin-finance.accounting.sales-returns.create') }}" class="btn btn-primary rounded d-flex align-items-center" style="gap: 0.5rem; padding: 0.5rem 1rem; height: 38px; min-height: 38px; line-height: 1.5; box-sizing: border-box; border: none; background: #D9251C; color: #ffffff; font-weight: 600; white-space: nowrap; box-shadow: 0 4px 10px rgba(217, 37, 28, 0.15);">
                                ...
                                <tbody>
                                    @forelse($returns as $return)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('admin-finance.sales-order.detail', $return->sales_order_id) }}" class="text-primary fw-bold">
                                                    {{ $return->salesOrder->so_number }}
                                                </a>
                                            </td>
                                            <td>
                                                {{ $return->salesOrder->customer->customer_name ?? 'Walk-in' }}
                                            </td>
                                            <td>
                                                {{ $return->return_date->format('M d, Y') }}
                                            </td>
                                            <td>
                                                <span class="badge bg-light text-dark border">{{ strtoupper($return->refund_method ?? 'N/A') }}</span>
                                            </td>
                                            ...
                                            <td class="text-center">
                                                <div class="d-flex justify-content-center align-items-center gap-1">
                                                    <a href="{{ route('admin-finance.accounting.sales-returns.show', $return->id) }}" class="btn btn-info shadow btn-xs sharp text-white" title="View details">
                                                        <i class="las la-eye"></i>
                                                    </a>
                                                    <form action="{{ route('admin-finance.accounting.sales-returns.destroy', $return->id) }}" method="POST" class="d-inline">
                                                        @csrf
                                                        @method('DELETE')
                                        ...
                                        <tr>
                                            <td colspan="9" class="text-center py-5">
                                                <div class="text-muted">
                                                    ...
                                                    <p class="mb-0 font-w600">No sales return transactions logged yet.</p>
                                                    <p class="small mb-0">Record a new return using the "Record Return" button above.</p>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforelse
