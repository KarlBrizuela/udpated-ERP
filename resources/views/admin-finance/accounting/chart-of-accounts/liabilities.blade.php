    ...
    <div class="card-body p-3 pt-1">
        <div class="row g-2">
            ...
            <div class="col-xl-3 col-md-4 col-sm-6">
                <div class="card h-100 shadow-sm hover-card liabilities-card" style="background-color: #ffffff; border: 1.5px solid #f59e0b !important; border-radius: 10px; transition: all 0.2s ease; cursor: pointer;" onclick="openAccountGroupDetailModal({{ $group->id }})">
                    <div class="card-body p-3 d-flex flex-column justify-content-between">
                        <div>
                            <div class="d-flex align-items-center justify-content-between mb-3">
                                <div class="rounded-circle d-flex align-items-center justify-content-center" style="width: 36px; height: 36px; background-color: rgba(245, 158, 11, 0.12); color: #f59e0b;">
                                    <i class="las la-layer-group fs-20"></i>
                                </div>
                                <span class="badge px-2.5 py-1 rounded-pill small fw-bold" style="background-color: rgba(245, 158, 11, 0.1); color: #f59e0b; font-size: 0.7rem;">
                                    {{ $group->accounts->count() }} {{ Str::plural('Account', $group->accounts->count()) }}
                                </span>
                            </div>
                            <h6 class="mb-1 fw-bold fs-15 text-warning" style="letter-spacing: -0.2px;">{{ $group->name }}</h6>
                            <p class="text-muted small mb-3" style="font-size: 0.76rem; line-height: 1.4; min-height: 38px;">{{ $group->description ?: 'Grouped Liability Accounts Card' }}</p>
                        </div>
                        <div class="pt-2 border-top d-flex justify-content-between align-items-center" style="border-color: #f1f5f9 !important;">
                            ...
                            <span class="fw-bold fs-14 text-warning">₱{{ number_format($group->calculated_balance, 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
